...
SOCIAL_TARGETS = ["LinkedIn", "Instagram", "Twitter", "Facebook"]
TONE_OPTIONS = ["Professionale", "Informativo", "Ispirazionale", "Umoristico"]
EDITOR_PLATFORMS = ["Instagram", "Facebook", "Twitter", "LinkedIn"]
...
def get_mock_metrics():
	return [
		{"label": "📝 Post Totali", "value": "128", "delta": "+6"},
	...
	with col_main:
		...
		with tabs[0]:
			with st.form("generate_form", clear_on_submit=False):
				...
				with col_a:
					social_target = st.selectbox("Social target", SOCIAL_TARGETS)
				with col_b:
					tone = st.selectbox("Tono", TONE_OPTIONS)
...
def render_calendar() -> None:
	...
	with col2:
		...
		with st.form("add_calendar_post"):
			...
			social = st.selectbox("Social", SOCIAL_TARGETS)
		...
		if post_to_edit:
			...
			with col2:
				edited_platform = st.selectbox(
					"Piattaforma",
					EDITOR_PLATFORMS,
					index=EDITOR_PLATFORMS.index(post_to_edit["platform"]),
				)
	...
	for post in st.session_state.posts:
		...
		with col1:
			if post["status"] == "programmato":
				if st.button("✏️ Modifica", key=f"edit_{post['id']}", use_container_width=True):
					st.session_state.editing_post = post["id"]
					st.rerun()
		with col2:
			if st.button("🗑️ Elimina", key=f"delete_{post['id']}", use_container_width=True):
